...
public interface Driver {
   ...
   */
  public default boolean verify(int decision, Map map, Vehicle vehicle) {
    try {
      ...
      switch (decision) {
        ...
        case 2: // Turn Norths
          if (vehicleIndex == 9) {
            for (Turn t : vehicle.currentRoad.to.getAvailableTurns()) {
  ...
  public default boolean checkTurn(Map map, Direction direction, Vehicle vehicle) {
    for (RoadSegment rs : map.getRoadSegments()) {
      if (rs.from.name.equals(vehicle.currentRoad.to.name) && rs.getLanes().get(0).direction == direction) {
   ...
   */
  public default void makeMove(int nextMove, Map map, Vehicle vehicle) {
  ...
  private void tryTurn(Map map, Direction d, Vehicle vehicle) {
    ...
    for (RoadSegment rs : map.getRoadSegments()) {
      if (rs.from.name.equals(vehicle.currentRoad.to.name) 
